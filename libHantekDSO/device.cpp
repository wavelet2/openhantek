...
namespace Hantek {
	...
	QString Device::search() {
		if(this->error)
			return tr("Can't search for Hantek oscilloscopes: %1").arg(libUsbErrorString(this->error));
		...
		if(deviceCount < 0)
			return tr("Failed to get device list: %1").arg(libUsbErrorString(errorCode));
		...
		if(this->model >= 0) {
			...
			if(errorCode == LIBUSB_SUCCESS) {
				...
				for(int interfaceIndex = 0; interfaceIndex < (int) configDescriptor->bNumInterfaces; ++interfaceIndex) {
					...
					if(interfaceDescriptor->bInterfaceClass == LIBUSB_CLASS_VENDOR_SPEC && interfaceDescriptor->bInterfaceSubClass == 0 && interfaceDescriptor->bInterfaceProtocol == 0 && interfaceDescriptor->bNumEndpoints == 2) {
						...
						if(errorCode < 0) {
							libusb_close(this->handle);
							this->handle = 0;
							message = tr("Failed to claim interface %1 of device %2: %3").arg(QString::number(interfaceDescriptor->bInterfaceNumber), deviceAddress, libUsbErrorString(errorCode));
						}
						else {
				...
				this->handle = 0;
				message = tr("Couldn't open device %1: %2").arg(deviceAddress, libUsbErrorString(errorCode));
			}
		}
		else
	...
	int Device::bulkCommand(Helper::DataArray<unsigned char> *command, int attempts) {
		...
		int errorCode = this->controlWrite(CONTROL_BEGINCOMMAND, this->beginCommandControl->data(), this->beginCommandControl->size());
		if(errorCode < 0)
			return errorCode;
		
		// Send bulk command
		return this->bulkWrite(command->data(), command->size(), attempts);
	}
	...
	int Device::getConnectionSpeed() {
		...
		errorCode = this->controlRead(CONTROL_GETSPEED, response.data(), response.size());
		if(errorCode < 0)
			return errorCode;
